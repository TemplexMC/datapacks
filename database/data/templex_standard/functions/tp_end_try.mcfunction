...
execute as @s[scores={Karma=..9999},tag=!bought_end] run tellraw @s [{"text":"You do not have enough Karma!\nYou need at least 10,000 Karma to buy this. Once you buy it, you have it forever.","color":"red"}]
execute as @s[scores={Karma=..9999},tag=!bought_end] run tellraw @a[tag=OP] [{"selector":"@s","color":"green"},{"text":" has tried to buy permanent access to the End, but does not have enough Karma.\n","color":"red"},{"text":"They currently have ","color":"gray"},{"score":{"name":"@s","objective":"Karma"},"color":"gold"},{"text":" Karma...","color":"gray"}]
# While having 10,000+ KP without having already bought access.
execute as @s[scores={Karma=10000..},tag=!bought_end] run tellraw @a [{"selector":"@s","color":"green"},{"text":" has just bought permanent access to the End for \u00A7610,000 \u00A74Karma \u00A74Points\u00A78.","color":"gold"}]
...
execute as @s[tag=bought_end] run function templex_standard:tp_end_confirm
# Reset.
