...
execute as @s[scores={SellItems=1..3}] run tellraw @a[tag=OP] [{"text":"Store","color":"gold"},{"text":" : ","color":"dark_gray"},{"selector":"@s","color":"green"},{"text":" just tried to sell 4 wheat (\u00A7680 \u00A7cKarma), but only had ","color":"red"},{"score":{"name":"@s","objective":"SellItems"},"color":"gold"},{"text":" wheat...","color":"red"}]
