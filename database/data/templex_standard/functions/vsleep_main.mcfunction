...
scoreboard players operation @e[name=Proxy1,type=armor_stand] Database = @e[name=Proxy2,type=armor_stand] Database
scoreboard players operation @e[name=Proxy2,type=armor_stand] Database -= @e[name=Proxy2,type=armor_stand] VoteDay
# Tellraw.
execute as @e[name=Proxy1,type=armor_stand,scores={Database=1}] run tellraw @a [{"selector":"@p[nbt={Sleeping:1b}]","color":"green"},{"text":" is now sleeping... Vote to turn it to day \u00A75[\u00A7bA \u00A7bscore \u00A7bof ","color":"gold"},{"score":{"name":"@e[name=Proxy2,type=armor_stand]","objective":"Database"},"color":"gold"},{"text":" \u00A7bis \u00A7bneeded \u00A7bfor \u00A7bday\u00A75]\u00A76:","color":"gold"}]
