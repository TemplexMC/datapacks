...
# Tellraw.
execute as @e[name=Proxy2,type=armor_stand,scores={Database=1}] run tellraw @a [{"selector":"@p[nbt={Sleeping:1b}]","color":"green"},{"text":" is now sleeping... Vote to turn it to day:","color":"gold"}]
execute as @e[name=Proxy2,type=armor_stand,scores={Database=1}] run tellraw @a [{"text":"[Turn to Day]","color":"green","hoverEvent":{"action":"show_text","value":"\u00A77Vote to turn it to day?"},"clickEvent":{"action":"run_command","value":"/trigger VoteDay"}},{"text":" "},{"text":"[Keep it Night]","color":"red","hoverEvent":{"action":"show_text","value":"\u00A77Want to keep it night?"},"clickEvent":{"action":"run_command","value":"/trigger VoteNight"}}]
# Enabling.
execute as @e[name=Proxy2,type=armor_stand,scores={Database=1}] run scoreboard players enable @a VoteDay
execute as @e[name=Proxy2,type=armor_stand,scores={Database=1}] run scoreboard players enable @a VoteNight
...
execute as @e[name=Proxy2,type=armor_stand,scores={Database=0}] run tellraw @a [{"text":"Players have voted to change to day! Changed to day.","color":"gray"}]
# Timer done.
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run tag @a remove VSLEEP
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run tag @a remove SleepVoted
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run scoreboard players reset @a VoteDay
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run scoreboard players reset @a VoteNight
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run scoreboard players reset @e[name=Proxy2,type=armor_stand] VoteDay
execute as @e[name=Proxy1,type=armor_stand,scores={Database=600}] run tellraw @a [{"text":"The majority of players do not want day right now... Keeping night.","color":"gray"}]
